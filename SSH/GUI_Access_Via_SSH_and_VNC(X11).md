...
=> To  create a `.ssh` in frienduser’s home as that user.
```bash
...
### 1. SSH into friend’s laptop with X11 forwarding:
```bash
...
* `frienduser@FRIEND_IP`--> username and the friend’s public IP or LAN IP.
...
xclock &
```
* `xeyes`, `xclock`, `gedit` — example GUI apps. The & runs them in the background so the shell is free.
...
```
* `-L 5901:localhost:5901` --> forward your local port `5901` to remote’s `localhost:5902` over SSH.
* `-p 22` --> port 
