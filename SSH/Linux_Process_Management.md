...
## 🔷**5. Cpu affnity (Bind Process to CPU Core):**
...
* Real-time processes can preempt normal ones (they get CPU time before any non–real-time process).
...
* A powerful kernel feature for resource control and isolation — used by Docker, systemd, etc.
...
```
=> Adds process with PID 1234 to the group — limits apply immediately.

### 🟣 `systemd-run`
* Launches a command as a transient systemd unit, with resource controls.
* Integrates with systemd’s cgroups.
...
* Similar to `chrt`, but more flexible — can set `SCHED_BATCH`, `SCHED_IDLE`, etc.
